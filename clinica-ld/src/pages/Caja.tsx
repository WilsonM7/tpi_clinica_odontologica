...
import { Search, X, CheckCircle, Pencil } from 'lucide-react'
import { format } from 'date-fns'

const CONTROL_ID = '__control__'

// Cuota base por material (valor del control futuro)
...
// Materiales disponibles para instalación
...
type Practica = { id: string; nombre: string; valor: number; multa: number | null; tipo_multa: string | null; cobra_descartable: boolean }
type Tratamiento = { id: string; tipo_material: string; tipo_tto: string; cuota_base: number; estado: string; especialidades?: { nombre: string } }
...
// Detecta si una práctica es de instalación
function esInstalacion(nombre: string) {
  return nombre.toLowerCase().includes('instalac')
}

// Extrae el material del nombre de la práctica
function materialDePractica(nombre: string): string {
  const n = nombre.toLowerCase()
  if (n.includes('zafiro')) return 'zafiro'
  if (n.includes('porcelana')) return 'porcelana'
  if (n.includes('metal')) return 'metalicos'
  if (n.includes('invisible') || n.includes('alineador')) return 'alineador invisible'
  if (n.includes('ortopedia')) return 'ortopedia'
  return ''
}

// Devuelve el material más caro entre dos
...
export default function Caja() {
  ...
  const [config, setConfig] = useState<Config>({ descartable: 3000, bracket: 5000, tubo: 6000, arco: 5000, recargo: 15 })
  ...
  useEffect(() => {
    // Reset instalación al cambiar práctica
  ...
  async function cargarDatosIniciales() {
    const [{ data: user }, { data: sucs }, { data: pracs }, { data: esps }, { data: profs }, { data: conf }] = await Promise.all([
      ...
      supabase.from('practicas').select('id, nombre, valor, multa, tipo_multa, cobra_descartable').eq('activa', true).order('nombre'),
    ...
    if (conf) {
      ...
      setConfig({
        descartable: get('descartable', 3000),
        bracket:     get('bracket', 5000),
        tubo:        get('tubo', 6000),
        arco:        get('arco', 5000),
        recargo:     get('recargo_tarjeta', 15) / 100,
      })
    }
  }
  ...
  async function cargarTratamientosActivos(pacienteId: string) {
    const { data } = await supabase.from('tratamientos')
      .select('id, tipo_material, tipo_tto, cuota_base, estado, especialidades(nombre)')
      .eq('paciente_id', pacienteId).in('estado', ['en tratamiento', 'en finalizacion'])
    setTratamientosActivos(data || [])
  }
  ...
  async function buscarPacientes(q: string) {
    ...
    const { data } = await supabase.from('pacientes')
      .select('id, apellido_nombre, dni').eq('activo', true)
      .or(`apellido_nombre.ilike.%${q}%,dni.ilike.%${q}%`).limit(8)
  ...
  function limpiarPaciente() {
    ...
    setTratamientosActivos([]); setTratamientoSeleccionado(null)
  ...
  // Descartable automático
  const tieneDescartable = esControlPractica || practicaActual?.cobra_descartable || false
  ...
  async function handleGuardar(e: React.FormEvent) {
    ...
    // Si es control y cambió el valor → actualizar cuota_base
    ...
    // Si es instalación → crear tratamiento
    let tratamientoId: string | null = null
    if (esInstalacionPractica) {
      // Determinar material final
      ...
      const espNombre = esOrtopedia ? 'ortopedia' : 'ortodoncia'
      const esp = especialidades.find(e => e.nombre.toLowerCase().includes(espNombre))

      // Cuota base del control futuro
    ...
    if (err) {
      ...
      if (form.fecha === fechaFiltro) await cargarCobrosGenerales()
            ...
            <form onSubmit={handleGuardar} className="bg-white rounded-xl border border-gray-200 p-5 space-y-4">
                ...
                <div>
                  ...
                  {tieneDescartable && (
                    <p className="text-xs text-blue-600 mt-1">
                      Incluye descartable: +{formatPeso(config.descartable)}
                    </p>
                  )}
                </div>
              </div>

              {/* Panel instalación de tratamiento */}
              {esInstalacionPractica && (
                <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 space-y-3">
                  <p className="text-sm font-semibold text-emerald-800">
                    🦷 Instalación — Se creará un nuevo tratamiento
                  </p>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="text-sm text-emerald-800 font-medium">Tratamiento híbrido (dos materiales diferentes)</span>
                  </label>

                  {esHibrido && (
                    <div>
                      <label className="block text-xs text-emerald-700 mb-1">Segundo material</label>
                      <select value={materialHibrido2}
                        onChange={e => setMaterialHibrido2(e.target.value)}
                      ...
                      {materialHibrido2 && (
                        <p className="text-xs text-emerald-700 mt-1">
                          Se guardará como <strong>Híbrido {materialBase} + {materialHibrido2}</strong> con cuota base {formatPeso(CUOTA_BASE_MAP[materialMasCaro(materialBase, materialHibrido2)] || 0)} (material más caro)
                        </p>
                      )}
                    </div>
                  )}

                  {/* Profesional que instala */}
                  <div>
                    <label className="block text-xs text-emerald-700 mb-1">Profesional que instala *</label>
                    <select value={profesionalInstalacion}
                      onChange={e => setProfesionalInstalacion(e.target.value)}
                      className="w-full border border-emerald-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
                      <option value="">-- Seleccioná --</option>
                      {profesionales.map(p => (
                      ...
                      {tratamientosActivos.map(t => (
                        <label key={t.id}
                          className={`flex items-center gap-3 p-2.5 rounded-lg border cursor-pointer ${
                            tratamientoSeleccionado?.id === t.id ? 'border-blue-500 bg-blue-100' : 'border-gray-200 bg-white hover:border-blue-300'
                          }`}>
                          <input type="radio" name="tratamiento"
                            checked={tratamientoSeleccionado?.id === t.id}
                            onChange={() => seleccionarTratamiento(t)}
                    ...
                    <div>
                      ...
                      {editandoValorControl ? (
                        <div className="space-y-1">
                          <input type="number" min="0" value={valorControl}
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[
                    ...
                    { key: 'arcos',    label: 'Arcos',    costo: config.arco },
                  ].map(mat => (
                    <div key={mat.key}>
                ...
                <div className="flex gap-2 flex-wrap">
                  {MEDIOS_PAGO.map(op => (
                    <button key={op.value} type="button"
                      ...
                      className={`flex-1 min-w-[80px] py-2 px-2 rounded-lg border text-xs font-medium transition-colors ${
                        form.medio_pago === op.value ? 'bg-blue-600 text-white border-blue-600' : 'border-gray-300 text-gray-600 hover:bg-gray-50'
                      }`}>
                      <div>{op.label}</div>
              ...
              <div className="bg-gray-50 rounded-lg p-4 space-y-2 text-sm">
                <div className="flex justify-between text-gray-600">
                  <span>{esControlPractica ? 'Control de tratamiento' : esInstalacionPractica ? 'Instalación' : 'Práctica'}</span>
                  <span>{formatPeso(subtotalPractica)}</span>
                </div>
                {tieneDescartable && (
                  <div className="flex justify-between text-blue-600">
                    <span>Descartable</span><span>+{formatPeso(montoDescartable)}</span>
                  </div>
                )}
                {subtotalMateriales > 0 && (
                  <div className="flex justify-between text-gray-600">
                    <span>Materiales</span><span>{formatPeso(subtotalMateriales)}</span>
                  </div>
                )}
                {tieneRecargo && (
                  <div className="flex justify-between text-orange-600">
                    <span>Recargo {form.medio_pago.toLowerCase()} ({config.recargo * 100}%)</span>
                    <span>+{formatPeso(recargo)}</span>
                  </div>
                )}
                {deudaAnteriorFinal > 0 && (
                  <div className="flex justify-between text-red-600">
                    <span>Deuda anterior</span><span>+{formatPeso(deudaAnteriorFinal)}</span>
                  </div>
                )}
              ...
              {exito && (
                <div className="flex items-center gap-2 text-green-600 bg-green-50 border border-green-200 rounded-lg px-4 py-3">
                  ...
                  <span className="text-sm font-medium">
                    Cobro registrado{esInstalacionPractica ? ' — Tratamiento creado correctamente' : ''}
                  </span>
                </div>
              )}
              <button type="submit" disabled={guardando}
                className="w-full bg-blue-600 text-white rounded-lg py-2.5 text-sm font-medium hover:bg-blue-700 disabled:opacity-50">
                {guardando ? 'Guardando...' : esInstalacionPractica ? 'Registrar instalación' : 'Registrar cobro'}
              </button>
            </form>
          )}
        </div>
                  ...
                  <div key={c.id} className="border border-gray-100 rounded-lg p-3 text-sm">
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <span className={`text-xs font-medium px-1.5 py-0.5 rounded-full ${
                        c.diferencia < 0 ? 'bg-red-100 text-red-600' :
                        c.diferencia > 0 ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-500'
                      }`}>
                        {c.diferencia < 0 ? `Debe ${formatPeso(Math.abs(c.diferencia))}` :
                         c.diferencia > 0 ? `A favor ${formatPeso(c.diferencia)}` : 'Saldado'}
                      </span>
                    </div>
        ...
        {cobrosGenerales.length > 0 && (
          <div className="grid grid-cols-3 gap-3 mb-4">
            <div className="bg-blue-50 rounded-lg px-4 py-3 text-center">
              ...
              <p className="font-bold text-blue-700 text-lg">{formatPeso(totalDia)}</p>
            </div>
            <div className="bg-green-50 rounded-lg px-4 py-3 text-center">
              <p className="text-xs text-gray-500 mb-0.5">Efectivo</p>
              <p className="font-bold text-green-700 text-lg">{formatPeso(totalEfectivo)}</p>
            </div>
            <div className="bg-orange-50 rounded-lg px-4 py-3 text-center">
              <p className="text-xs text-gray-500 mb-0.5">Electrónico</p>
              <p className="font-bold text-orange-700 text-lg">{formatPeso(totalElectronico)}</p>
            </div>
          </div>
        )}
        {cobrosGenerales.length === 0 ? (
                  ...
                  <tr key={c.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2.5">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                        c.medio_pago === 'EFECTIVO' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'
                      }`}>{c.medio_pago}</span>
                    </td>
                    ...
                    <td className={`px-4 py-2.5 text-right font-medium ${
                      c.diferencia < 0 ? 'text-red-600' : c.diferencia > 0 ? 'text-green-600' : 'text-gray-400'
                    }`}>
                      {c.diferencia === 0 ? '—' :
                       c.diferencia < 0 ? `-${formatPeso(Math.abs(c.diferencia))}` :
                       `+${formatPeso(c.diferencia)}`}
                    </td>
                  </tr>
                ))}
              </tbody>
